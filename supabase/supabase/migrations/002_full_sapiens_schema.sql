-- Migration: 001_full_sapiens_schema.sql
-- Descrição: Estrutura completa de tabelas, triggers, RLS e funções para o SapiensStudying
...
-- ============================================================
-- 1. TABELA PRINCIPAL: STUDY_PLANS
-- ============================================================
...
-- ============================================================
-- 2. TABELA DE SEMANAS (PLAN_WEEKS)
-- ============================================================
...
-- ============================================================
-- 3. TABELA DE ATIVIDADES DIÁRIAS (PLAN_TASKS)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_plan_tasks_week ON plan_tasks(week_id, day_number);
...
-- ============================================================
-- FUNÇÕES E TRIGGERS DE PROGRESSO AUTOMÁTICO
-- ============================================================

-- Função para recalcular o progresso da semana e do plano
...
BEGIN
  ...
  -- 2. Calcula progresso do plano geral
  SELECT COUNT(*), COUNT(*) FILTER (WHERE completed = TRUE)
...
-- ============================================================
-- FUNÇÃO RPC: TOGGLE TASK COMPLETION
-- ============================================================
...
BEGIN
  ...
  -- Busca plano atualizado pelo trigger
...
-- ============================================================
-- FUNÇÃO RPC: INSERÇÃO ATÔMICA DE PLANO GERADO POR IA
-- ============================================================
...
BEGIN
  ...
  v_estimated_end_date := p_payload->>'estimatedEndDate';
  ...
  -- 3. Insere Recursos Curados
  v_resources := p_payload->'resources';
  IF v_resources IS NOT NULL AND jsonb_array_length(v_resources) > 0 THEN
    FOR v_res IN SELECT * FROM jsonb_array_elements(v_resources)
    LOOP
      INSERT INTO plan_resources (
        ...
        v_plan_id,
        COALESCE(v_res->>'category', 'livros'),
        v_res->>'title',
        v_res->>'authorOrCreator',
        v_res->>'badge',
        COALESCE(v_res->>'badgeColor', 'primary'),
        v_res->>'imageUrl',
        COALESCE((v_res->>'rating')::NUMERIC, 4.8),
        COALESCE(v_res->>'ratingCount', '1k'),
        v_res->>'url',
        v_res->>'description'
      );
    END LOOP;
  ...
  IF v_methods IS NOT NULL AND jsonb_array_length(v_methods) > 0 THEN
    ...
    LOOP
      INSERT INTO plan_practices (
        ...
        v_plan_id,
        COALESCE(v_method->>'methodType', 'flashcards'),
        COALESCE(v_method->>'tag', 'Repetição Ativa'),
        v_method->>'title',
        v_method->>'description',
        v_method->>'badge',
        v_method->>'meta',
        COALESCE(v_method->>'actionText', 'Iniciar')
      );
    END LOOP;
  END IF;
...
-- Políticas de leitura e escrita públicas/anon e autenticadas
